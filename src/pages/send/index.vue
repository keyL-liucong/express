<template>
	<view class="app-container">
		...
		<view class="send-part-box">
			<view class="send-part-title">收件方式</view>
			<!--收件方式-->
			<view class="mode">
				<view class="send-mode-radio">
			...
			<view class="row" v-if="is_flag == 0" @click="navTo('/pages/address/index?currentTab=0&from=send')">
				...
				<view class="address" @click="navTo('/pages/address/index?currentTab=0&from=send')">
					地址簿
				</view>
			</view>
			<!-- 收件地址 -->
			...
			<view class="row" v-if="is_flag == 1" @click="navTo('/pages/address/self?from=send')">
				...
				<view class="send-info" v-else>
					<view class="row">
						<text>{{ selfAddr.realname }}</text>
						<text>{{ selfAddr.mobile }}</text>
					</view>
					<view class="row gray clamp-1"> {{ selfAddr.address }} </view>
				</view>
				<view class="address" @click="navTo('/pages/address/self?from=send')">
					自提点
				</view>
			</view>
			<!-- 自提点 -->
		...
		<view class="send-info-box">
			<tui-list-cell :hover="false">
				<view class="thorui-input-item">
					...
					<input v-if="declareList.length === 0" class="thorui-input" placeholder="请申报您寄送的物品信息>"
						placeholder-class="thorui-phcolor" v-model="name" disabled="true"
			...
			<tui-list-cell :hover="false">
				<view class="thorui-input-item">
					...
					<input class="thorui-input" placeholder="请输入包裹的预估重量" placeholder-class="thorui-phcolor"
						v-model="weightNum" disabled="true" @click="handlePopup('weight')" />
			...
			<tui-list-cell :hover="false">
				<view class="thorui-input-item">
					...
					<input class="thorui-input" placeholder="补充体积预估费用更准确" placeholder-class="thorui-phcolor"
						v-model="volume" disabled="true" @click="handlePopup('volume')" />
					<text>KG</text>
				</view>
			</tui-list-cell>
			<tui-list-cell :hover="false">
				<view class="thorui-input-item">
					...
					<input class="thorui-input" placeholder="贵重物品建议购买保价服务>" placeholder-class="thorui-phcolor"
						v-model="submitStatementValue_msg" disabled="true" @click="handlePopup('support')" />
				</view>
			</tui-list-cell>
			<tui-list-cell :hover="false">
				<view class="thorui-input-item">
					...
					<input class="thorui-input" placeholder="选择优惠券>" placeholder-class="thorui-phcolor" disabled="true"
						v-model="coupon.coupon_name" @click="navTo('/pages/coupon/index?from=order')" />
				</view>
			</tui-list-cell>
			<view class="upload-img">
				...
				<view class="img-list">
					<tui-upload :value="value" :serverUrl="serverUrl" :header="header" limit=3 width=150 height=150
						@complete="uploadComplete" @remove="uploadRemove"></tui-upload>
				</view>
			</view>
		</view>
			...
			<view class="row line">
				...
				<view class="row-right">
					<radio-group @change="radioChange">
						<label class="thorui-radio thorui-align__center thorui-padding" style="margin-right: 20rpx">
		...
		<!-- 底部弹框部分 -->
		<tui-bottom-popup backgroundColor="#fff" @close="popup" :show="popupShow">
				...
				<view class="type-1-list-wrap">
					<view class="type-1-list">
						<view v-if="firstList.length > 0">
							<view class="type-item" v-for="(item, index) in firstList" :key="index"
								@click="handleSecList(item.declare_id)">{{ item.declare_name }}</view>
						</view>
						<view v-else>
							<view class="type-item" v-for="(item, index) in secList" :key="index"
								@click="handleItemPrice(item)">{{ item.declare_name }}</view>
						</view>
					</view>
				</view>
				<button>提交</button>
		...
		<tui-datetime :type="1" ref="dateTime" @confirm="change" :startYear="startYear" :endYear="endYear">
		</tui-datetime>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async handleSecList(declare_id) {
				let res = await this.$api.getSecList({
					page: 1,
					size: 100,
					declare_id
				});
			...
			onRodeChange() {
				if (!this.total_amounts) {
					...
					this.submitStatementValue = parseFloat((this.fillStatementValue * this.order_insured_price) / 100)
						.toFixed(2);
			...
			// 获取一级物品申报
			async getFirstList() {
				let res = await this.$api.getFirstList({
					page: 1,
					size: 100
				});
